...
#pragma region TVTComment
			...
			tvtComment.reset(new TVTComment::TVTComment(m_pApp, &commentWindow_, s_.tvtComment));
#pragma endregion
			...
			/* if (hPanel_) {
				...
				hForce_ = CreateWindowEx(WS_EX_WINDOWEDGE | WS_EX_TOOLWINDOW, TEXT("ru.jk.force"), TEXT("TvtComment"),
...
#pragma region TVTComment
	...
	if (path[0] && !_tcschr(TEXT("/\\"), path[0]) && path[1] != TEXT(':')) {
		...
		if (GetLongModuleFileName(g_hinstDLL, dir, _countof(dir))) {
			s_.tvtComment = dir;
			size_t lastSep = s_.tvtComment.find_last_of(TEXT("/\\"));
			if (lastSep != tstring::npos) {
				s_.tvtComment.erase(lastSep + 1);
			}
			s_.tvtComment += path;
		} else {
			s_.tvtComment.clear();
